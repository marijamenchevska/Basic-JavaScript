...
let changeButton = document.getElementById("changeBackground");
...
function PrintExcercises(inputObject, numOfProps) {
    ...
    else if (isNaN(numOfProps)) {
        alert("Please enter a number.");
    }
    else if (Number(numOfProps) <= 0) {
        alert("Please enter a positive number.");
    }
    else {
        values = Object.values(inputObject);
        if (Number(numOfProps) > values.length) {
            alert("Too many exercises, give the students some time to learn!!");
        }
        else {
            container.innerHTML = `<p id="exerciseText"> Vezba ${numOfProps}: ${values[numOfProps - 1]}</p>`;
            container.innerHTML += `---------------------------------------------------`;

            let exercise = document.getElementById("exerciseText");
            exercise.style.fontSize = "25px";
            exercise.style.fontFamily = "Lucida Sans";
        }   
    }
}
